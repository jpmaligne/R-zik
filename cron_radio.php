...
$content = "";
$content .= "<icecast\n";
$content .= "    <location>Earth</location>\n";
$content .= "    <admin>icemaster@localhost</admin>\n";
$content .= "    <limits>\n";
...
$content .= "    </authentication>\n";
$content .= "    <hostname>localhost</hostname>\n";
...
for($i=0;$i<$nb_user;$i++){
    if((json_decode($response)[$i]->password_live)!= ""){
        $content .= "<mount>\n";
        $content .= "    <mount-name>/".json_decode($response)[$i]->id."</mount-name>\n";
